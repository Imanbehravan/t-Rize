...
housing_dataset = pd.read_csv(f'/home/iman/projects/kara/Projects/T-Rise/xgboost-comprehensive/subset_{args.partition_id + 1}.csv')
...
if args.centralised_eval:
    ...
    global_test_data = pd.read_csv('/home/iman/projects/kara/Projects/T-Rise/xgboost-comprehensive/global_test_data.csv')
    unique_values = global_test_data['ocean_proximity'].unique()
    value_to_number = {value: idx for idx, value in enumerate(unique_values)}

    # Replace unique values with numbers
    global_test_data['ocean_proximity'] = global_test_data['ocean_proximity'].map(value_to_number)

# Ensure all columns are numeric and drop rows with missing values
    global_test_data = global_test_data.apply(pd.to_numeric, errors='coerce').dropna()
    valid_data = dict()
    valid_data['inputs'] = global_test_data.drop(columns='median_house_value')
    valid_data['label'] = global_test_data['median_house_value']
